...
V.lst <- diag(solve(H.df.lst))

wald.lst <- replicate(3,mle.lst) + cbind(rep(0,3),-1,1) * qnorm(1-alpha/2) * replicate(3, sqrt(V.lst))
CIs <- data.frame(round(cbind(wald.lst,rbind(CI.df.lst, CI.mu.lst, CI.sigma.lst)),3)); colnames(CIs) <- c("MLE", "Wald lower", "Wald upper", "Profile lower", "Profile upper"); rownames(CIs) <- c("df", "mu", "sigma")
CIs
