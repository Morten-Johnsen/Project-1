...
Two approaches: 1) Box-Cox transformation 2) fit distributions directly to the standardized pow.obs.
```{r, fig.height=7}
...
prof.b0 <- function(beta0, x = log.data$AIDS_yes[2], n = log.data$n[2]){ #duplicate of nll.p_0
  p <- exp(beta0)/(1+exp(beta0))
  return(-sum(dbinom(x, size = n, prob = p, log = T)))
}

prof.b1 <- function(beta1, beta0, x = log.data$AIDS_yes[1], n = log.data$n[1]){ #duplicate of nll.p_1
...
Comparing these to eachother, we see that the Wald CI is a very good approximation of the actual CIs. However, when comparing the estimates from our glm model we see that the $95\%$ CI for AZT is wider for the model estimate...
