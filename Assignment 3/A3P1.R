...
combined.nll <- function(theta, pow.obs, ws){
  beta0 <- theta[1]
  beta1 <- theta[2]
  beta2 <- theta[3]
  phi <- theta[4]
  ...
  n <- length(e) - 1
  px_given_xminus1 <- n/2 * log(sigma) + 1/(2*sigma) * sum((e[-1]-phi*e[-(n+1)])^2)
  return(px + px_given_xminus1)
}

combined.opt <- nlminb(start = c(1,0,0,0,1)
                       , objective = combined.nll
