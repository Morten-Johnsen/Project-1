...
```{r}
AIC(betaModel)
AIC(ws.and.ws.squared) #hmm, der er jo noget, som ikke passer her
g_inverse <- function(y,lambda){
  return((1/(exp(y*lambda)+1))^(1/lambda) * exp(y))
}
#2*norm.lin.par$objective+2*4
logLik(betaModel)#compare this to the one below
-norm.lin.par$objective+sum(log(-1/(D$pow.obs.norm*(-1+D$pow.obs.norm^0.26)))) #actual log-likelihood. It's greater than that of the beta model. This corresponds to the figure above :))
AIC.bm <- -2*logLik(betaModel)+2*( length(betaModel$coefficients$mean) + length(betaModel$coefficients$precision) )
AIC.normTrans <- 2*(norm.lin.par$objective-sum(log(-1/(D$pow.obs.norm*(-1+D$pow.obs.norm^0.26))))) + 2*length(norm.lin.par$par) #aha, så passer det jo :)
round(rbind(AIC.bm,AIC.normTrans),digits=4)
```
...
$$
l(y) = \sum_{i=1}^n logf_y(y) +\sum_{i=1}^nlog\left(\dfrac{-1}{g^{-1}(y)(-1+g^{-1}(y)^\lambda)}\right)
$$
