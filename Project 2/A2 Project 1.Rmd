...
#here, the log likelihood L(y) consists of two terms: -sum(log(f(y))) and -sum(log(dg/dx)). Thus -log(f(y)) = L(y) + log(dg/dx) and now the
#likelihoods from the two different models (beta and normal) can be compared:
logLik.normTrans <- -norm.lin.par$objective+sum(log(-1/(D$pow.obs.norm*(-1+D$pow.obs.norm^0.26)))) #actual log-likelihood. It's greater than that of the beta model. This corresponds to the figure above :))
