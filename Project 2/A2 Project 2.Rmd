...
loglogistic.model.manual.AIC <- - 2 * loglogistic.model.manual$objective + 2 * length(loglogistic.model.manual$par)
...
plot(fit.exp$time, -log(fit.exp$surv), main = "Exponential Model")
abline(a=0, b=1)

#slide 51 week 7
actg$cox.snell.wei <- exp((log(actg$time) - weibull.model$linear.predictors) / weibull.model$scale)
fit.wei <- survfit(Surv(cox.snell.wei, event == 1) ~ 1, data = actg)
plot(fit.wei$time, -log(fit.wei$surv), main = "Weibull Model")
abline(a=0, b=1)

#slide 55 week 7 + slide 33 week 7 + slide 65 week 7
actg$cox.snell.loglog <- log( 1 + exp((log(actg$time) - log.logistic.model$linear.predictors) / log.logistic.model$scale))
fit.loglog <- survfit(Surv(cox.snell.loglog, event == 1) ~ 1, data = actg)
plot(fit.loglog$time, -log(fit.loglog$surv), main = "Log-logistic Model")
abline(a=0, b=1)
```
Weibull and log-logistic seem quite similar. We test if the scale parameter of the weibull.

Use AIC to compare the models (exp excluded due to poor cox-snell fit):
```{r}
2 * weibull.model$loglik[2] + 2 * (3 + 1) #3 parms + scale

2 * log.logistic.model$loglik[2] + 2 * (3 + 1) #3 parms + scale
```
...
confint(weibull.model)
```

And for the scale parameter:
```{r}
exp(0.24834)
exp(0.24834 + c(-1,1)*qnorm(0.975)*0.09715)
```
...
exp(50*0.02063062)
#95% CI
exp(c(50*0.01319748, 50*0.02806376))
```
...
time.steps <- seq(0,max(actg$time),0.5)
...
plot(survival.functions, conf.int = T, col = 2:5, ylim = c(0.75,1), lwd = 1)
grid()
