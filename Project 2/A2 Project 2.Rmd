...
wald.p.value <- 2*pnorm(waldTestStatistic) #Skal det her være dnorm i stedet måske?
...
# Ved ikke om det er mere passende at finde score functions ud fra de enkelte parametre? Dette har dog ingen
#effekt på den derpåfølgende konklusion.
# score.beta0 <- function(theta){
#   time = actg$time
#   event = actg$event
#   treatment = actg$tx
#   cd4 = actg$cd4
...
#   beta2 <- theta[4]
# 
#   out <- -sum(-event/sigma + time^(1/sigma)*exp(-(beta0+beta1*treatment+beta2*cd4)/sigma)/sigma)
#   return(out)
# }
#
# score.beta1 <- function(theta){
#   time = actg$time
#   event = actg$event
#   treatment = actg$tx
#   cd4 = actg$cd4
...
#   beta2 <- theta[4]
# 
#   out <- -sum(-event*treatment/sigma + time^(1/sigma)*treatment*exp(-(beta0+beta1*treatment+beta2*cd4)/sigma)/sigma)
#   return(out)
# }
# 
# score.beta2 <- function(theta){
#   time = actg$time
#   event = actg$event
#   treatment = actg$tx
#   cd4 = actg$cd4
...
#   beta2 <- theta[4]
# 
#   out <- -sum(-event*cd4/sigma + time^(1/sigma)*cd4*exp(-(beta0+beta1*treatment+beta2*cd4)/sigma)/sigma)
#   return(out)
# }
...
Based on this calculation of the time ratio of the treatment effect, it is evident that the treatment increase the median survival time by a factor 2.32 95% CI [1.53; 3.68].
...
cat("Time Ratio for the treatment effect: TR(tx = 1, tx = 0) = ", exp(beta2*50), " 95% CI [",exp(c(50*beta2.lower)),",", exp(c(50*beta2.upper)),"]")
```

Here we see that by increasing the cd4 count by 50, the median survival time is increased by a factor 2.8 95% CI [2.26, 3.60].
