...
log.logistic.model <- survreg(Surv(time, event) ~ tx + cd4, data = actg, dist = "loglogistic")
```
...
```{r}
2 * weibull.model$loglik[1] + 2 * (3 + 1) #3 parms + scale

2 * log.logistic.model$loglik[1] + 2 * (3 + 1) #3 parms + scale
```
