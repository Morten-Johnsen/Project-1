...
```{r}
cat("MLE: (shape; scale) = (",round(mle.wd30[1],3),"; ",round(mle.wd30[2],3),")\n\n\n")
...
ppB <- ggplot(D)+
  ...
  ylim(c(0,5))+
  annotate( "text", x = 4/5*max(D$pow.obs.norm), y = c(4.5, 4.1), label = temp, parse = T  ) +
  scale_colour_manual(values = "black")+
...
temp1 <- paste("k == ", round(mle.ws30.weib[1],3),"(",round(wald.ws30.shape.hessian.matrix.based,3)[1],",",round(wald.ws30.shape.hessian.matrix.based,3)[2],")")
temp2 <- paste("lambda == ", round(mle.ws30.weib[2],3), "  (",round(wald.ws30.scale.hessian.matrix.based,3)[1], ",",round(wald.ws30.scale.hessian.matrix.based,3)[2],")")
temp3 <- paste("E[X] == ", round(par.repar.wei$par[1],3))
temp4 <- paste("V[X] == ", round(par.repar.wei$par[2],3))
...
ps <- ggplot(D)+
  ...
  annotate( "text", x = 4/5*max(D$ws30), y = c(0.09, 0.082, 0.074, 0.066), label = temp, parse = T  ) +
  scale_colour_manual(values = "black")+
  ggtitle("Wind Speed")+
