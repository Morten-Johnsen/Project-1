...
#wald m. matrix
H.pow.shapes.m <- hessian(func = l.pow.funp.beta, x = par.beta$par, data = D$pow.obs.norm)
sd.pow.shapes.m <- sqrt(diag(solve(H.pow.shapes.m)))
wald.pow.shape1.m <- par.beta$par[1] + c(-1,1) * qnorm(1-alpha/2) * sd.pow.shapes.m[1]
wald.pow.shape2.m <- par.beta$par[2] + c(-1,1) * qnorm(1-alpha/2) * sd.pow.shapes.m[2]

round( rbind( CI.pow1, wald.pow.shape1, wald.pow.shape1.m, CI.pow2, wald.pow.shape2, wald.pow.shape2.m, mle.pow ), digits = 3 )
```
...
#wald m. matrix
H.ws30.m <- hessian(func = nl.ws30.funp, x = par.ws30$par, data = D$ws30)
sd.ws30.m <- sqrt(diag(solve(H.ws30.m)))
wald.ws30.shape.m <- par.ws30$par[1] + c(-1,1) * qnorm(1-alpha/2) * sd.ws30.m[1]
wald.ws30.scale.m <- par.ws30$par[2] + c(-1,1) * qnorm(1-alpha/2) * sd.ws30.m[2]

round( rbind( CI.ws30.shape, wald.ws30.shape, wald.ws30.shape.m, CI.ws30.scale, wald.ws30.scale,
              wald.ws30.scale.m, mle.ws30.weib ), digits = 3 )
...
H.ws30.E.m <- hessian(func = repar.l.ws30.funp, x = repar.E.ws30$par, data = D$ws30)
sd.ws30.E.m <- sqrt(diag(solve(H.ws30.E.m)))
wald.ws30.E.m <- repar.E.ws30$par[2] + c(-1,1) * qnorm(1-alpha/2) * sd.ws30.E.m[2]

round( rbind( wald.ws30.E, wald.ws30.E.m), digits = 3);repar.E.ws30$par[2]
...
#wald m. matrix
H.wd30.m <- hessian(func = nl.wd30.funp, x = wrapped.cauc.par$par,  data = D$wd30)
sd.wd30.m <- sqrt(diag(solve(H.wd30.m)))
wald.wd30.mu.m <- wrapped.cauc.par$par[1] + c(-1,1) * qnorm(1-alpha/2) * sd.wd30.m[1]
wald.wd30.rho.m <- wrapped.cauc.par$par[2] + c(-1,1) * qnorm(1-alpha/2) * sd.wd30.m[2]

round( rbind( CI.wd30.mu, wald.wd30.mu, wald.wd30.mu.m, CI.wd30.rho, wald.wd30.rho, wald.wd30.rho.m,
              mle.wd30 ), digits = 3 )
```
...
temp1 <- paste("alpha == ", round(theta.hat.y$par[1], 4)) #
temp2 <- paste("beta == ", round(theta.hat.y$par[2], 3)) #
temp <- c(temp1, temp2)

ppN <- ggplot(D)+
  geom_histogram(aes(x = transformed.pow.obs.norm, y = ..density..), colour='white', alpha=0.6, bins=30)+
  theme_bw()+
  stat_function(aes(colour = "Normal Distribution"), fun = dnorm, n = dim(D)[1], args = list(mean = theta.hat.y$par[1], sd = theta.hat.y$par[2]))+
  annotate( "text", x = 4/5*max(D$transformed.pow.obs.norm), y = c(0.11, 0.1), label = temp, parse = T  ) +
...
round( rbind( wald.pow.shape1.m, wald.pow.shape2.m, wald.ws30.scale.m,
              wald.ws30.shape.m, wald.wd30.mu.m, wald.wd30.rho.m ), digits = 3 ) #wald matrix
